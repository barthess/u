...
/* ћожно вносить поправку пр€мо в метку времени, но точное врем€ запуска
 * может понадобитьс€ позднее, поэтому заведем отдельную переменную */
static int64_t Correction = 0;

/* количество переполнений системного таймера. ‘актически дл€ программного
 * расширени€ разр€дности счетчика на 32 бита */
static uint32_t WrapCount = 0;

/* последнее значение счетчика дл€ отлова момента переполнени€ */
